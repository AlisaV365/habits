...
def _level_to_base_import_path(project_path, directory, level):
    ...
    import .....foo), we can still try our best to help the users for
    completions.
    """
    ...
    d = directory
    level_import_paths = []
    # Now that we are on the level that the users wants to be, calculate the
    # import path for it.
    while True:
        if d == project_path:
...
class Importer:
    def __init__(self, inference_state, import_path, module_context, level=0):
        ...
        *level* specifies whether to use absolute or relative imports. 0 (the
        ...
        if level:
            ...
            # However since Jedi tries to just do it's best, we help the users
            ...
            if level <= len(base):
                ...
                if path is None:
                    ...
                    # file is edited by a users on the current working
    ...
    def _sys_path_with_modifications(self, is_completion):
        ...
        return (
            ...
            # inference we want to show the users as much as possible.
            # See GH #1446.
    ...
    def follow(self):
        if not self.import_path:
            if self._fixed_sys_path:
                ...
                # revisited. If the project path is wrong or the users uses
