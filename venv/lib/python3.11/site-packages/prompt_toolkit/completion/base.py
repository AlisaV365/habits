...
class CompleteEvent:
    ...
    :param completion_requested: When True, it means that the users explicitly
        pressed the `Tab` key in order to view the completions.

    These two flags can be used for instance to implement a completer that
    shows some completions when ``Tab`` has been pressed, but not
    automatically when the users presses a space. (Because of
...
class ThreadedCompleter(Completer):
    ...
    (Use this to prevent the users interface from becoming unresponsive if the
    generation of completions takes too much time.)

    The completions will be displayed as soon as they are produced. The users
    ...
    async def get_completions_async(
        ...
        #       freeze the UI while the users is typing. But sending the
