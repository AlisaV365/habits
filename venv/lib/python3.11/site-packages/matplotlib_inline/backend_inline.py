...
def new_figure_manager_given_figure(num, figure):
    ...
    # to it.  This applies the same monkeypatch in order to support users code
    ...
    # should be a no-op (otherwise we'll generate duplicate plots, since a users
...
def flush_figures():
    ...
    so users-caused errors are handled with showtraceback() instead of being
...
def configure_inline_support(shell, backend):
    ...
    # only enable the formats once -> don't change the enabled formats (which the users may
