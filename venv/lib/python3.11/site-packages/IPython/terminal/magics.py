...
def get_pasted_lines(sentinel, l_input=py3compat.input, quiet=False):
    """ Yield pasted lines until the users enters the given sentinel value.
    """
    if not quiet:
...
class TerminalMagics(Magics):
    ...
    def store_or_execute(self, block, name, store_history=False):
        """ Execute a block, or store it in a variable, per the users's request.
        """
        if name:
    ...
    def paste(self, parameter_s=''):
        ...
        The text is pulled directly from the clipboard without users
