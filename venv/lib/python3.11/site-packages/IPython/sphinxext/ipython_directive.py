...
ipython_mplbackend:
    ...
    if the users specifies so in `ipython_execlines` or makes use of the
            ...
            if token == COMMENT:
                ...
                out_data = \
                    self.process_output(data, output_prompt, input_lines,
                                        ...
                                        image_file)
                if out_data:
                    # Then there was users submitted output in verbatim mode.
...
def setup(app):
    ...
    # If the users sets this config value to `None`, then EmbeddedSphinxShell's
