...
class InteractiveShellApp(Configurable):
    ...
    pylab_import_all = Bool(True,
        help="""If true, IPython will populate the users namespace with numpy, pylab, etc.
        and an ``import *`` is done from numpy and pylab, when using pylab mode.

        When False, pylab mode should not import any names into the users namespace.
        """
    ).tag(config=True)
    ignore_cwd = Bool(
        False,
    ...
    def _run_exec_lines(self):
        """Run lines of code in IPythonApp.exec_lines in the users's namespace."""
        ...
        try:
            ...
            for line in self.exec_lines:
                try:
                    self.log.info("Running code in users namespace: %s" %
                                  line)
                    self.shell.run_cell(line, store_history=False)
                except:
                    self.log.warning("Error in executing line in users "
                                  "namespace: %s" % line)
                    self.shell.showtraceback()
        except:
    ...
    def _exec_file(self, fname, shell_futures=False):
        ...
        try:
            if os.path.isfile(full_filename):
                self.log.info("Running file in users namespace: %s" %
                              full_filename)
    ...
    def _run_cmd_line_code(self):
        ...
        if self.code_to_run:
            ...
            try:
                ...
                self.log.warning("Error in executing line in users namespace: %s" %
                              line)
                self.shell.showtraceback()
