...
Prefilters transform users input before it is exec'd by Python.  These
...
class PrefilterManager(Configurable):
    ...
    The IPython prefilter is run on all users input before it is run.  The
    ...
    def prefilter_line(self, line, continue_prompt=False):
        ...
        if not line:
            # Return immediately on purely empty lines, so that if the users
    ...
    def prefilter_lines(self, lines, continue_prompt=False):
        ...
        This covers cases where there are multiple lines in the users entry,
        which is the case when the users goes back to a multiline history
...
class PrefilterTransformer(Configurable):
    """Transform a line of users input."""
...
class AssignmentChecker(PrefilterChecker):
    ...
    def check(self, line_info):
        """Check to see if users is assigning to a var for the first time, in
...
class AutoMagicChecker(PrefilterChecker):
    ...
    def check(self, line_info):
        ...
        users namespace which could shadow it."""
...
class PrefilterHandler(Configurable):
    ...
    def handle(self, line_info):
        ...
        # don't want to force the users to have to backspace all the way to
