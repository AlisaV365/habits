...
class InteractiveShell(SingletonConfigurable):
    ...
    ast_transformers = List([], help=
        ...
        to users input before code is run.
        """
    ).tag(config=True)
    ...
    def init_environment(self):
        """Any changes we need to make to the users's environment."""
        pass
    ...
    def init_virtualenv(self):
        """Add the current virtualenv to sys.path so the users can import modules from it.
        This isn't perfect: it doesn't use the Python interpreter with which the
        virtualenv was built, and it ignores the --no-site-packages option. A
        warning will appear suggesting the users installs IPython in the
    ...
    def init_hooks(self):
        # hooks holds pointers used for users-side customizations
        self.hooks = Struct()
        ...
        for hook_name in hooks.__all__:
            # default hooks have priority 100, i.e. low; users hooks should have
            # 0-100 priority
    ...
    def set_hook(self, name, hook, priority=50, str_key=None, re_key=None):
        ...
        IPython exposes some of its internal API as users-modifiable hooks.  By
    ...
    def new_main_mod(self, filename, modname):
        """Return a new 'main' module object for users code execution.
    ...
    def init_create_namespaces(self, user_module=None, user_ns=None):
        # Create the namespace where the users will operate.  user_ns is
        ...
        # FIXME. For some strange reason, __builtins__ is showing up at users
        ...
        # A record of hidden variables we have added to the users namespace, so
        # we can list later only variables defined in actual interactive use.
        self.user_ns_hidden = {}

        # Now that FakeModule produces a real module, we've run into a nasty
        # problem: after script execution (via %run), the module where the users
    ...
    def prepare_user_module(self, user_module=None, user_ns=None):
        """Prepare the module and namespace in which users code will be run.
        ...
        user_module : module, optional
            The current users module in which IPython is being run. If None,
    ...
    def init_sys_modules(self):
        ...
        # this because they need to manage the users local/global namespaces
    ...
    def init_user_ns(self):
        """Initialize all users-visible namespaces to their minimum defaults.

        Certain history lists are also initialized here, as they effectively
        act as users namespaces.
        ...
        # rest of what we *do* want the users to see with %who even on a new
        # session (probably nothing, so they really only see their own stuff)

        # The users dict must *always* have a __builtin__ reference to the
        ...
        # make global variables for users access to the histories
        ns['_ih'] = self.history_manager.input_hist_parsed
        ns['_oh'] = self.history_manager.output_hist
        ns['_dh'] = self.history_manager.dir_hist

        # users aliases to input and output histories.  These shouldn't show up
        ...
        # putting anything here, as we really want %who to show the users their
        # stuff, not our variables.

        # Finally, update the real users's namespace
        self.user_ns.update(ns)

    @property
    def all_ns_refs(self):
        """Get a list of references to all the namespace dictionaries in which
        IPython might store a users-created object.
    ...
    def reset(self, new_session=True, aggressive=False):
        ...
        # Restore the users namespaces to minimal usability
        self.init_user_ns()
        ...
        # Restore the default and users aliases
    ...
    def del_var(self, varname, by_name=False):
        ...
        by_name : bool
            ...
            namespace. If False (default), find the variable in the users
    ...
    def push(self, variables, interactive=True):
        """Inject a group of variables into the IPython users namespace.

        Parameters
        ----------
        variables : dict, str or list/tuple of str
            The variables to inject into the users's namespace.  If a dict, a
        ...
        # Propagate variables to users namespace
    ...
    def drop_by_id(self, variables):
        """Remove a dict of variables from the users namespace, if they are the
        same as the values in the dictionary.

        This is intended for use by extensions: variables that they've added can
        be taken back out if they are unloaded, without removing any that the
        users has overwritten.
    ...
    def init_traceback_handlers(self, custom_exceptions):
        ...
        # and add any custom exception handlers the users may have specified
    ...
    def set_custom_exc(self, exc_tuple, handler):
        ...
        if handler is None:
            ...
            def wrapped(self,etype,value,tb,tb_offset=None):
                """wrap CustomTB handler, to protect IPython from users code
    ...
    def excepthook(self, etype, value, tb):
        ...
        any app directly invokes sys.excepthook, it will look to the users like
    ...
    def showtraceback(self, exc_tuple=None, filename=None, tb_offset=None,
        ...
        should be used throughout the code for presenting users tracebacks,
    ...
    def set_custom_completer(self, completer, pos=0) -> None:
        ...
        and return a list with current completions to suggest to the users.
        """
    ...
    def run_line_magic(self, magic_name: str, line, _stack_depth=1):
        ...
        if fn is None:
            ...
            # Note: this is the distance in the stack to the users's frame.
    ...
    def auto_rewrite_input(self, cmd):
        """Print to the screen the rewritten form of the users's command.
        ...
        after the users's input prompt.  This helps the users understand that the
    ...
    def _format_user_obj(self, obj):
        """format a users object to display dict
    ...
    def user_expressions(self, expressions):
        """Evaluate a dict of expressions in the users's namespace.
    ...
    def ex(self, cmd):
        """Execute a normal python statement in users namespace."""
        with self.builtin_trap:
            exec(cmd, self.user_global_ns, self.user_ns)

    def ev(self, expr):
        """Evaluate python expression expr in users namespace.
    ...
    def run_cell(
        ...
        store_history : bool
            ...
            history. For users code calling back into IPython's machinery, this
    ...
    def transform_ast(self, node):
        ...
        node : ast.Node
            ...
            produced by parsing users input.
    ...
    async def run_code(self, code_obj, result=None, *, async_=False):
        ...
        # Set our own excepthook in case the users code tries to call it
    ...
    def enable_matplotlib(self, gui=None):
        ...
        gui : optional, string
            ...
            users's matplotlibrc configuration file).  Note that not all backends
    ...
    def enable_pylab(self, gui=None, import_all=True, welcome_message=False):
        ...
        # We want to prevent the loading of pylab to pollute the users's
    ...
    def var_expand(self, cmd, depth=0, formatter=DollarFormatter()):
        ...
        The global namespace for expansion is always the users's interactive
        namespace.
        """
    ...
    def find_user_code(self, target, raw=True, py_only=False, skip_encoding_cookie=True, search_ns=False):
        ...
        target : str
            ...
            string or Macro in the users namespace.
        ...
        try:                                              # User namespace
            ...
            raise ValueError(("'%s' was not found in history, as a file, url, "
                                "nor in the users namespace.") % target) from e
    ...
    def _atexit_once(self):
        ...
        if not getattr(self, "_atexit_once_called", False):
            ...
            # Clear all users namespaces to release all references cleanly.
